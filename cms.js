...
const instance = cms({
  ...
  addons: {
    ...
    getContentColor: (backgroundColor) =>
      contrast(backgroundColor) === 'dark' ? '#fff' : '#000',
  },
  globals: {
    fonts,
    hostname,
  ...
  shortcodes: {
    ...
    video: (attrs, page) => {
      ...
      const type = `video/${attrs.video.split('.').slice(-1).join('')}`;
      const modifiers = [];
      if (attrs.bordered) {
        modifiers.push('bordered');
      }
      ...
      }${attrs.muted ? ` muted` : ''}${attrs.loop ? ` loop` : ''} playsinline>
    ...
    gallery: (attrs, page) => {
      ...
      return `
        <span class="gallery gallery--${attrs.variant || 'responsive'}${
        modifiers.length
          ? ` ${modifiers.map((modifier) => `gallery--${modifier}`).join(' ')}`
          : ''
      }">
          ${items
            .map((itemName) => {
    ...
    term: (attrs, page) => {
      const terms = page.genesis.findPageByUrl(`/glossary`).children;
      const match = terms.find((term) =>
        [term.title, term.alias].includes(attrs.term),
      );
      if (!match) {
        throw new Error(`Missing term: ${attrs.term}`);
      }
      return `<a href="${match.url}">${attrs.text || attrs.term}</a>`;
    },
    math: (attrs) =>
    ...
    github: (attrs) =>
      `<a href="https://github.com/${
        attrs.github
      }" class="link link--github" target="_blank" rel="noopener noreferrer">${
        attrs.text || attrs.github
      }</a>`,
    npm: (attrs) =>
